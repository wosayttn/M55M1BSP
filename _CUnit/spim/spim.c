...
/**
  ...
  */
int SPIM_IsWriteDone(uint32_t u32NBit)
{
    uint8_t status[1];
...
int SPIM_InitFlash(int clrWP)
{
    ...
    // Becuse not usre in SPI or QPI mode, do QPI reset and then SPI reset.
    // QPI Reset Enable
    ...
    {
        SPIM_DRV_DEBUG("Flash initailze failed!!\n");
        g_MID = MFGID_UNKNOW;
        return -1;
    }
...
/**
  ...
  * @return     None.
  */
void SPIM_WriteInPageDataByIo(uint32_t u32Addr, int is4ByteAddr, uint32_t u32NTx, uint8_t *pu8TxBuf, uint32_t wrCmd,
...
/**
  ...
  * @return     None.
  */
void SPIM_WriteInPageDataByPageWrite(uint32_t u32Addr, int is4ByteAddr, uint32_t u32NTx, uint8_t *pu8TxBuf,
                                     uint32_t wrCmd, int isSync)
{
    ...
    {
        SPIM_SetQuadEnable(1, 1);                             // Set Quad Enable.
        SPIM_SetQpiMode(1);                                   // Enter QPI mode.
    }
    ...
    {
        SPIM_SetQpiMode(0);                                   // Exit QPI mode.
    }
}
