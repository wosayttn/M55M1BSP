...
#if defined(USE_FMC) || defined(NO_PRINTF)
#define ERROR_MSG(...)
#define printf(...)
#else
...
/************** arm_cmse.h       **************/
/**********************************************/
/* For TC8263(M55M1): cmse_check_address_range will take the address of ITCM (0x00000000) as NULL */
static void *__attribute__((__always_inline__))
_cmse_check_address_range(void *__pb, size_t __s, int __flags)
{
    ...
    switch (__flags & ~(CMSE_MPU_UNPRIV | CMSE_MPU_NONSECURE)) {
        ...
        case CMSE_MPU_READWRITE | CMSE_AU_NONSECURE:
            return __permb.flags.nonsecure_readwrite_ok ? __pe : NULL;

        case CMSE_MPU_READ | CMSE_AU_NONSECURE:
            return __permb.flags.nonsecure_read_ok ? __pe : NULL;

        case CMSE_AU_NONSECURE:
            return __permb.flags.secure ? NULL : __pe;
#endif

        case CMSE_MPU_READ | CMSE_MPU_READWRITE:
        case CMSE_MPU_READWRITE:
            return __permb.flags.readwrite_ok ? __pe : NULL;

        case CMSE_MPU_READ:
            return __permb.flags.read_ok ? __pe : NULL;
...
enum dma350_lib_error_t dma350_runcmd(struct dma350_ch_dev_t *dev,
...
#elif 1 // Test by CCTu, always enable 
    dma350_ch_enable_intr(dev, DMA350_CH_INTREN_DONE);
#endif
...
enum dma350_lib_error_t dma350_get_memattr(void *address,
        ...
        bool writable)
{
    ...
    if ((Selected_MPU->CTRL & MPU_CTRL_ENABLE_Msk) &&
        ...
        /* For TC8263(M55M1): If addrsss bit[28] is set, use non-secure access */
...
#else
    dev->cfg.ch_base->CH_SRCTRANSCFG = 0x000F0800;
#endif
    // printf("dev->cfg.ch_base->CH_SRCTRANSCFG = 0x%08X\n", dev->cfg.ch_base->CH_SRCTRANSCFG);
        ...
        void *des)
{
    ...
    dma350_ch_set_desmemattr(dev, memattr.mpu_attribute,
                             memattr.mpu_shareability);
#else
    dev->cfg.ch_base->CH_DESTRANSCFG = 0x000F0800;
#endif
    // printf("dev->cfg.ch_base->CH_DESTRANSCFG = 0x%08X\n", dev->cfg.ch_base->CH_DESTRANSCFG);
        ...
        uint32_t des_size)
{
    ...
    /* For TC8263(M55M1): cmse_check_address_range will take the address of ITCM (0x00000000) as NULL */
    if (NULL == _cmse_check_address_range((void *)src, src_size, CMSE_MPU_READ)) {
        return DMA350_LIB_ERR_RANGE_NOT_ACCESSIBLE;
    }

    /* For TC8263(M55M1): cmse_check_address_range will take the address of ITCM (0x00000000) as NULL */
    if (NULL == _cmse_check_address_range(des, des_size, CMSE_MPU_READWRITE)) {
        return DMA350_LIB_ERR_RANGE_NOT_ACCESSIBLE;
    }
