...
/* File Contents:                                                                                          */
/*   sc_cunit.c                                                                                            */
/*            The test function of SC for CUnit.                                                           */
/* Project:   M2354                                                                                        */
/*---------------------------------------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------------------------------------*/
...
{    
    ...
    /* Check SC_SET_VCC_PIN() */
        ...
        SC_SET_VCC_PIN(pSC[port+NS_idx], SC_PIN_STATE_HIGH); CLK_SysTickDelay(100);
        CU_ASSERT_NOT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_PWREN_Msk), 0x0);          
        SC_SET_VCC_PIN(pSC[port+NS_idx], SC_PIN_STATE_LOW); CLK_SysTickDelay(100);
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_PWREN_Msk), 0x0);          

    /* Check SC_SET_CLK_PIN() */
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_CLKKEEP_Msk), 0x0);          
        SC_SET_CLK_PIN(pSC[port+NS_idx], SC_CLK_ON); CLK_SysTickDelay(100);
        CU_ASSERT_NOT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_CLKKEEP_Msk), 0x0);          
        SC_SET_CLK_PIN(pSC[port+NS_idx], SC_CLK_OFF); CLK_SysTickDelay(100);
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_CLKKEEP_Msk), 0x0);          

    /* Check SC_SET_IO_PIN() */
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_SCDATA_Msk), 0x0);          
        SC_SET_IO_PIN(pSC[port+NS_idx], SC_PIN_STATE_HIGH); CLK_SysTickDelay(100);
        CU_ASSERT_NOT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_SCDATA_Msk), 0x0);          
        SC_SET_IO_PIN(pSC[port+NS_idx], SC_PIN_STATE_LOW); CLK_SysTickDelay(100);
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_SCDATA_Msk), 0x0);          

    /* Check SC_SET_RST_PIN() */
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_RSTEN_Msk), 0x0);          
        SC_SET_RST_PIN(pSC[port+NS_idx], SC_PIN_STATE_HIGH); CLK_SysTickDelay(100);
        CU_ASSERT_NOT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_RSTEN_Msk), 0x0);          
        SC_SET_RST_PIN(pSC[port+NS_idx], SC_PIN_STATE_LOW); CLK_SysTickDelay(100);
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->PINCTL&SC_PINCTL_RSTEN_Msk), 0x0); 

    /* Check SC_SET_STOP_BIT_LEN() */
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->CTL&SC_CTL_NSB_Msk), 0x0);          
        SC_SET_STOP_BIT_LEN(pSC[port+NS_idx], 1); CLK_SysTickDelay(100);
        CU_ASSERT_NOT_EQUAL((pSC[port+NS_idx]->CTL&SC_CTL_NSB_Msk), 0x0);          
        SC_SET_STOP_BIT_LEN(pSC[port+NS_idx], 2); CLK_SysTickDelay(100);
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->CTL&SC_CTL_NSB_Msk), 0x0);          
    ...
    /* Check SC_SetTxRetry() */
        ...
        SC_SetTxRetry(pSC[port+NS_idx], 0); while(pSC[port+NS_idx]->CTL&SC_CTL_SYNC_Msk){}
        CU_ASSERT_EQUAL((pSC[port+NS_idx]->CTL&(SC_CTL_TXRTYEN_Msk|SC_CTL_TXRTY_Msk)), 0x0);          
    ...
    /* Check SC_WRITE(), SC_READ() by INTERNAL Loop Back Test */
        SC_Open(pSC[port+NS_idx], SC_PIN_STATE_IGNORE, SC_PIN_STATE_HIGH); CLK_SysTickDelay(100);
        pSC[port+NS_idx]->PINCTL |= BIT31; CLK_SysTickDelay(100);
        ...
        pSC[port+NS_idx]->PINCTL &= ~BIT31; CLK_SysTickDelay(100);
        SC_Close(pSC[port+NS_idx]); CLK_SysTickDelay(100);
    }
...
{    
    ...
    /* Check SC_ClearFIFO() by INTERNAL Loop Back Test */
        ...
        SC_ClearFIFO(pSC[port+NS_idx]); CLK_SysTickDelay(100);
    ...
    /* Check SC_Open(), SC_Close() and SC_IsCardInserted() */
        ...
        SC_Open(pSC[port+NS_idx], SC_PIN_STATE_IGNORE, SC_PIN_STATE_LOW); CLK_SysTickDelay(100);
