...
void API_TIMER_StartAndStop(void)
{
    ...
    /* Check TIMER_TRGSRC_TIMEOUT_EVENT, TIMER_TRGSRC_CAPTURE_EVENT */
    for(i=0; i<4; i++) {         
        TIMER_SetTriggerSource(TimerCh[i], TIMER_TRGSRC_CAPTURE_EVENT);
        CU_ASSERT_EQUAL(TimerCh[i]->TRGCTL, 0x1);
        //printf("%d\n", TimerCh[i]->TRGCTL);
        TIMER_SetTriggerSource(TimerCh[i], TIMER_TRGSRC_TIMEOUT_EVENT);
        CU_ASSERT_EQUAL(TimerCh[i]->TRGCTL, 0x0);
        //printf("%d\n", TimerCh[i]->TRGCTL);
    }
    
    /* Check TIMER_TRG_TO_PWM, TIMER_TRG_TO_ADC, TIMER_TRG_TO_PDMA */
    for(i=0; i<4; i++) {         
        TIMER_SetTriggerTarget(TimerCh[i], TIMER_TRG_TO_PWM);
        CU_ASSERT_EQUAL(TimerCh[i]->TRGCTL, BIT1);
        
        TIMER_SetTriggerTarget(TimerCh[i], TIMER_TRG_TO_EADC);
        CU_ASSERT_EQUAL(TimerCh[i]->TRGCTL, BIT2);
        
        TIMER_SetTriggerTarget(TimerCh[i], TIMER_TRG_TO_DAC);
        CU_ASSERT_EQUAL(TimerCh[i]->TRGCTL, BIT3);

        TIMER_SetTriggerTarget(TimerCh[i], TIMER_TRG_TO_PDMA);
