...
static uint32_t  g_bDoISPFFTest = FALSE;
...
{
    uint32_t u32Addr;
    int32_t i32Active, i32RetCode = 0;
    int32_t i32TimeOutCnt;
...
int32_t FMC_WriteConfig(uint32_t u32ConfigAddr, uint32_t u32ConfigVal)
{
    ...
    if (FMC_Write(u32ConfigAddr, u32ConfigVal) != 0)
    {
        FMC_DISABLE_CFG_UPDATE();
        return -1;
    }
...
int32_t FMC_EnableAPROMWriteProtect(uint16_t u16BlockIdx, uint16_t u16BlockCnt)
{
    FMC->APWPROT[u16BlockIdx >> 5] |= (1ul << u16BlockIdx);
