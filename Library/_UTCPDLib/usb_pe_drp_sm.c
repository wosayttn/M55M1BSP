...
#else
static void print_current_state(const int port, char *pString)
{
#if (USB_PD_DEBUG_LABELS ==1)
    const char *mode = "";
...
static void pe_src_ready_run(int port)
{
    ...
    if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
    {
        ...
        for (i = 0; i < cnt; i = i + 1)
        {
            uint32_t *u32Ptr = &rx_emsg[port].buf + i * 4;
            DBG_PRINTF("0x%x\n", *u32Ptr);
        }
    ...
    switch (parse_vdm_response_common(port))
    {
        case VDM_RESULT_WAITING:
            ...
            if (PE_CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR))
            {
                ...
                PE_CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
                set_state_pe(port, get_last_state_pe(port));
                DBG_PRINTF("VDM Error\n");
            }
            else
                DBG_PRINTF("VDM Waitting\n");
...
static void pe_vdm_response_entry(int port)
{
    ...
    switch (vdo_cmd)
    {
        case CMD_DISCOVER_IDENT:
            DBG_PRINTF("Discover ID\n");
            func = svdm_rsp.identity;
            break;

        case CMD_DISCOVER_SVID:
            DBG_PRINTF("Discover SVID\n");
            func = svdm_rsp.svids;
            break;

        case CMD_DISCOVER_MODES:
            DBG_PRINTF("Discover MODE\n");
            func = svdm_rsp.modes;
            break;

        case CMD_ENTER_MODE:
            DBG_PRINTF("Enter Mode\n");
