 ...
 */
    ...
    #define DBG_PRINTF_ENTRY printf
#else
    #define DBG_PRINTF(...)
#endif

#ifdef DBG_VDM_PRINTF
    #define VDM_PRINTF printf
#else
    #define VDM_PRINTF(...)
#endif

#define DBGPRINTF_VCS(...)  //printf
...
#define PE_CHK_FLAG(port, flag) (pe[port].flags & (flag))
    ...
    #define PE_SET_FLAG2(port, flag) atomic_or(&pe[port].flag2s, (flag))
    #define PE_CLR_FLAG2(port, flag) atomic_clear_bits(&pe[port].flag2s, (flag))
    #define PE_CHK_FLAG2(port, flag) (pe[port].flag2s & (flag))
#endif
/*
...
#if (CONFIG_TC8260_FRS == 1)
    #define PE_FLAG2S_FRS_DETECT               BIT(0)
#endif
...
enum usb_pe_state
{
    ...
    /* SW Add For SRC to SNK Fast Role Swap */
...
__maybe_unused static __const_data const char *const pe_state_names[] =
{
...
#if (CONFIG_USB_PD_EXTENDED_MESSAGES == 1)
    ...
    /* SW Add For SRC to SNK Fast Role Swap */
    [PE_FRS_SRC_SNK_EVALUATE_SWAP] = "PE_FRS_SRC_SNK_Evaluate_Swap"
...
#if (CONFIG_USBC_VCONN == 0) || (CONFIG_USB_PD_REV30 == 0)
...
#if (CONFIG_USB_PD_EXTENDED_MESSAGES == 0)
...
//#ifdef CONFIG_USB_PD_EXTENDED_MESSAGES
#if (CONFIG_USB_PD_EXTENDED_MESSAGES == 1)  //2022/04/12 for C99
    //GEN_NOT_SUPPORTED(PE_SRC_CHUNK_RECEIVED);
    //#define PE_SRC_CHUNK_RECEIVED PE_SRC_CHUNK_RECEIVED_NOT_SUPPORTED
    //GEN_NOT_SUPPORTED(PE_SNK_CHUNK_RECEIVED);
    //#define PE_SNK_CHUNK_RECEIVED PE_SNK_CHUNK_RECEIVED_NOT_SUPPORTED
...
void pe_set_explicit_contract(int port)
{
    ...
    /* SW ADD for notification UTCPD event {*/
    UTCPD_NotifyEvent(port, UTCPD_PD_CONTRACT, 1);
    /* SW ADD for notification UTCPD event }*/
...
void pe_invalidate_explicit_contract(int port)
{
    ...
    /* SW ADD for notification UTCPD event {*/
    UTCPD_NotifyEvent(port, UTCPD_PD_CONTRACT, 0);
    /* SW ADD for notification UTCPD event }*/
}
...
static bool pe_can_send_sop_prime(int port)
{
    if (IS_ENABLED(CONFIG_USBC_VCONN))
    {
        if (PE_CHK_FLAG(port, PE_FLAGS_EXPLICIT_CONTRACT))
        {
            if (prl_get_rev(port, TCPCI_MSG_SOP) == PD_REV20)
            {
                DBG_PRINTF("CBL: Contract-VCONN SRC? && ROLE_DFP?\n");
                return tc_is_vconn_src(port) &&
                       pe[port].data_role == PD_ROLE_DFP;
            }
            else
            {
                DBG_PRINTF("CBL: Contract-VCONN SRC?\n");
                return tc_is_vconn_src(port);
            }
        }
        else
        {
            DBG_PRINTF("CBL: No Contract-VCONN SRC? ROLE_SRC?\n");
            return tc_is_vconn_src(port) &&
    ...
    {
        DBG_PRINTF("CBL: Didn't Support VCONN\n");
        return false;
    }
}
...
void pe_ps_reset_complete(int port)
{
    ...
    /* SW Add to disable VBUS and VCONN */
    //{
...
static bool common_src_snk_dpm_requests(int port)
{
    ...
    else if (PE_CHK_DPM_REQUEST(port, DPM_REQUEST_PORT_DISCOVERY))
    {
        ...
        if (!PE_CHK_FLAG(port, PE_FLAGS_MODAL_OPERATION))
        {
            ...
            DBG_PRINTF("%s enable discover_identity\n", __FUNCTION__);
...
static bool source_dpm_requests(int port)
{
    ...
    if (pe[port].dpm_request)
    {
        ...
        DBG_PRINTF("SRC DPM REQUEST 0x%x\n", dpm_request);
                               ...
                               DPM_REQUEST_DR_SWAP))
        {
            ...
            return true;
                                    ...
                                    DPM_REQUEST_PR_SWAP))   /* Prepare to send "PR_SWAP" message */
        {
            pe_set_dpm_curr_request(port,
                                    DPM_REQUEST_PR_SWAP);
    ...
    return false;
#else
    return false;   /*   No any src DPM request */
...
static bool sink_dpm_requests(int port)
{
    ...
    {
        uint32_t dpm_request = pe[port].dpm_request;
        DBG_PRINTF("SNK DPM REQUEST 0x%x\n", dpm_request);
        PE_SET_FLAG(port, PE_FLAGS_LOCALLY_INITIATED_AMS);
    ...
    return false;
#else
    return false; /* No ant SNK DPM req */
#endif /* #if (CONFIG_SUPPORT_SNK == 1)  */
}
...
#else
static void print_current_state(const int port, char const *pString)
{
#if (USB_PD_DEBUG_LABELS ==1)
    const char *mode = "";
...
static void send_source_cap_swex0(int port, int sop_type)
{
    const uint32_t *src_pdo;
    const int src_pdo_cnt = dpm_get_source_pdo(&src_pdo, port);
    ...
    {
        /* No source capabilities defined, sink only */
        send_ctrl_msg(port, sop_type, PD_CTRL_REJECT);
    }

    tx_emsg[port].len = src_pdo_cnt * 4;
    memcpy(tx_emsg[port].buf, (uint8_t *)src_pdo, tx_emsg[port].len);

    send_data_msg(port, sop_type, PD_DATA_SOURCE_CAP);
}
void send_source_cap_swex(int port, int sop_type)
{
    send_source_cap_swex0(port, sop_type);
}
...
static void pe_send_request_msg(int port)
{
    ...
    printf("C%d: Req [%d] %dmV %dmA", port, RDO_POS(rdo),
...
__maybe_unused static bool pe_attempt_port_discovery(int port)
{
    ...
    if (pd_timer_is_expired(port, PE_TIMER_DISCOVER_IDENTITY))
    {
        ...
        {
            pe[port].tx_type = TCPCI_MSG_SOP_PRIME;
            DBG_PRINTF("pe_attempt_port_discovery: PE_VDM_IDENTITY_REQUEST_CBL \n");
...
#ifdef SW    /* Call from usb_pe_drp_sm.c */
...
static enum pe_msg_check pe_sender_response_msg_run(const int port)
{
    ...
    if (pd_timer_is_disabled(port, PE_TIMER_SENDER_RESPONSE))
    {
        ...
        if (PE_CHK_FLAG(port, PE_FLAGS_TX_COMPLETE))
        {
             ...
             */
            DBG_PRINTF("%%%% enable PE_TIMER_SENDER_RESPONSE timer\n");
...
static void pe_src_startup_entry(int port)
{
    ...
    else
    {
         ...
         */
        DBG_PRINTF("%s enable discover_identity timer\n", __FUNCTION__);
...
#ifdef SW /* Remove it due to ChargeLAB Power Z didn't reply it */
...
static void pe_src_discovery_entry(int port)
{
    ...
    if (get_last_state_pe(port) != PE_VDM_IDENTITY_REQUEST_CBL)
    {
        ...
        DBG_PRINTF("last pe state isn't  PE_VDM_IDENTITY_REQUEST_CBL\n");
    }
    else
    {
        DBG_PRINTF("last pe state is PE_VDM_IDENTITY_REQUEST_CBL\n");
    }
...
static void pe_src_discovery_run(int port)
{
    ...
    if (pd_timer_is_expired(port, PE_TIMER_SOURCE_CAP))
    {
        ...
        {
            set_state_pe(port, PE_SRC_SEND_CAPABILITIES);
            DBG_PRINTF("set_state_pe(port, PE_SRC_SEND_CAPABILITIES)\n");
            return;
        }
        else if (!PE_CHK_FLAG(port, PE_FLAGS_PD_CONNECTION))
        {
            set_state_pe(port, PE_SRC_DISABLED);
            DBG_PRINTF("set_state_pe(port, PE_SRC_DISABLED) 1\n");
            return;
        }
    }
                ...
                N_DISCOVER_IDENTITY_PRECONTRACT_LIMIT))
    {
        pe[port].tx_type = TCPCI_MSG_SOP_PRIME;
        set_state_pe(port, PE_VDM_IDENTITY_REQUEST_CBL);
        DBG_PRINTF("pe_src_discovery_run: PE_VDM_IDENTITY_REQUEST_CBL \n");
        return;
    }
    ...
    {
        set_state_pe(port, PE_SRC_DISABLED);
        DBG_PRINTF("set_state_pe(port, PE_SRC_DISABLED) 2\n");
        return;
    }
...
//static void enable_not_auto_reply_goodcrc(int port)
//{
//    outp32(UTCPD0_BASE+UTCPD_NOGCRC, inp32(UTCPD0_BASE+UTCPD_NOGCRC) | 0x1);
//}

//static void disable_not_auto_reply_goodcrc(int port)
//{
//    outp32(UTCPD0_BASE+UTCPD_NOGCRC, inp32(UTCPD0_BASE+UTCPD_NOGCRC) & ~0x1);
//}
...
void clone_utcpd_reg(uint32_t Target)
{
    ...
    for (i = 0; i < 0x180 / 4; i = i + 4)
    {
        *pTarget = inp32(UTCPD0_BASE + i);
        pTarget += 1;
    }
}
#endif
...
#if (CONFIG_DUMP_REGISTER == 1)
    /* Under coding for check REQ can't receive well after BIST message. */
    outp32(UTCPD0_BASE + TCPC_REG_FAULT_CTRL, 0x13);
    outp32(UTCPD0_BASE + TCPC_REG_POWER_CTRL, 0x8);

    if ((inp32(UTCPD0_BASE + 0x2C) == 0x25) && (inp32(UTCPD0_BASE + 0x38) == 0x09))
    {
        //Wait stable
        if (u32target == 0)
        {
...
static void pe_src_send_capabilities_run(int port)
{
    ...
    {
        set_state_pe(port, PE_SRC_READY);
        DBG_PRINTF("MSG DPM DISCARDED\n");
        return;
    }
    else if (msg_check == PE_MSG_DISCARDED)
    {
        DBG_PRINTF("MSG PE DISCARDED\n");
        pe_send_soft_reset(port, TCPCI_MSG_SOP);
        return;
    }
    ...
    if (msg_check == PE_MSG_SEND_COMPLETED)
    {
        ...
        printf("REV 0x%x\n", inp32(UTCPD0_BASE + TCPC_REG_RX_HDR));
    }
     ...
     */
    DBG_PRINTF("CHK SNK Req\n");

    if ((msg_check & PE_MSG_SENT) &&
            PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
    {
        DBG_PRINTF("MSG Receive\n");
        PE_CLR_FLAG(port, PE_FLAGS_MSG_RECEIVED);
...
static void pe_src_send_capabilities_exit(int port)
{
    ...
    /* SW ADD */
    //disable_not_auto_reply_goodcrc(port);
#endif /* #if (CONFIG_SUPPORT_SRC == 1) */
}
...
static void pe_src_transition_supply_entry(int port)
{
    ...
    if (PE_CHK_FLAG(port, PE_FLAGS_ACCEPT))
    {
        ...
        DBG_PRINTF("Prepare Accept msg\n");


#if 1 /* remove to pe_src_transition_supply_run- "PD_CTRL_ACCEPT" TX Complete */
        /* tester will check VBUS keep same value as receive ACCEPT */
        /* SW Add code to enable Sink VBUS path */
        printf("RDO = %d\n", pe[port].requested_idx);
        VBUS_Source_Level(port, pe[port].requested_idx);
#endif
    }
    else
    {
        send_ctrl_msg(port, TCPCI_MSG_SOP, PD_CTRL_GOTO_MIN);
        DBG_PRINTF("Prepare GotoMin msg\n");


        /* SW Add code. Go to Min is 5V ???? not sure */
        DBG_PRINTF("Min REQ IDX = %d\n", pe[port].requested_idx);
...
static void pe_src_ready_entry(int port)
{
    ...
    //print_current_state(port, __FUNCTION__);
    printf("%s\n", __FUNCTION__);
    DBG_PRINTF("%s as CC\n", (inp32(UTCPD0_BASE + TCPC_REG_TCPC_CTRL)&TCPC_REG_TCPC_CTRL_PLUG_ORIENTATION) ? "CC2" : "CC1");
...
static uint32_t src_reay_count = 0;
static void pe_src_ready_run(int port)
{
#if (CONFIG_SUPPORT_SRC == 1)
    uint32_t i;

    src_reay_count = src_reay_count + 1;

    //if((src_reay_count%200) == 0)
    //printf("src\n");
    /*
    ...
    if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
    {
        ...
        PE_CLR_FLAG(port, PE_FLAGS_MSG_RECEIVED);
        DBG_PRINTF("SRC Rec %d\n", cnt);

        for (i = 0; i < cnt; i = i + 1)
        {
            uint32_t *u32Ptr = (uint32_t *)&rx_emsg[port].buf + i * 4;
            DBG_PRINTF("0x%x\n", (uint32_t)*u32Ptr);
        }
        ...
        if (ext > 0)
        {
            switch (type)
            {
                ...
                case PD_EXT_GET_BATTERY_CAP:
                    DBG_PRINTF("PD_EXT_GET_BATTERY_CAP\n");
                    set_state_pe(port, PE_GIVE_BATTERY_CAP);
                    break;

                case PD_EXT_GET_BATTERY_STATUS:
                    DBG_PRINTF("PD_EXT_GET_BATTERY_STATUS\n");
        ...
        else if (cnt > 0)
        {
            switch (type)
            {
                case PD_DATA_REQUEST:
                    DBG_PRINTF("PD_DATA_REQUEST\n");
                    set_state_pe(port, PE_SRC_NEGOTIATE_CAPABILITY);
                    return;

                case PD_DATA_SINK_CAP:
                    DBG_PRINTF("PD_DATA_SINK_CAP\n");
                    break;

                case PD_DATA_VENDOR_DEF:
                    DBG_PRINTF("PD_DATA_VENDOR_DEF\n");
        ...
        else
        {
            switch (type)
            {
                ...
                case PD_CTRL_PING:
                    DBG_PRINTF("PD_CTRL_PING\n");
                    break;

                case PD_CTRL_GET_SOURCE_CAP:
                    DBG_PRINTF("PD_CTRL_GET_SOURCE_CAP\n");
                    set_state_pe(port, PE_SRC_SEND_CAPABILITIES);
                    return;

                case PD_CTRL_GET_SINK_CAP:
                    DBG_PRINTF("PD_CTRL_GET_SINK_CAP\n");
                    set_state_pe(port, PE_SNK_GIVE_SINK_CAP);
                    return;

                case PD_CTRL_GOTO_MIN:
                    DBG_PRINTF("PD_CTRL_GOTO_MIN\n");
                    break;

#if (OPT_DRP == 1)

                case PD_CTRL_PR_SWAP:
                    DBG_PRINTF("PD_CTRL_PR_SWAP\n");      /* Received "PR_SWAP" Message from partner side */
                    set_state_pe(port,
                                 PE_PRS_SRC_SNK_EVALUATE_SWAP);
                    return;
                ...
                case PD_CTRL_FR_SWAP:
                    DBG_PRINTF("PD_CTRL_FR_SWAP\n");      /* Received "FR_SWAP" Message from partner side */
                    set_state_pe(port,                /* Only SRC side can receive FR_SWAP message */
                                 PE_FRS_SRC_SNK_EVALUATE_SWAP);
                    return;
                    /* SW Add } */
#endif
                ...
                case PD_CTRL_WAIT:
                case PD_CTRL_PS_RDY:
                    DBG_PRINTF("PD_CTRL_ACCEPT\n");
                    DBG_PRINTF("PD_CTRL_REJECT\n");
                    DBG_PRINTF("PD_CTRL_WAIT\n");
                    DBG_PRINTF("PD_CTRL_PS_RDY\n");
                    pe_send_soft_reset(port,
                                       PD_HEADER_GET_SOP(rx_emsg[port].header));
                    return;
...
static void pe_src_disabled_entry(int port)
{
     ...
     */
    /* SW Add */
    for (i = 0; i < pe[port].src_cap_cnt; i = i + 1)
        pe[port].src_caps[i] = 0;

    for (i = 0; i < pe[port].snk_cap_cnt; i = i + 1)
        pe[port].snk_caps[i] = 0;
...
#else
        /* In sink role, we didn't start up DR_SWAP and VCS_SWAP */
        PE_CLR_FLAG(port, PE_FLAGS_DR_SWAP_TO_DFP);
        PE_CLR_FLAG(port, PE_FLAGS_VCONN_SWAP_TO_ON);
#endif
    }
...
static void pe_snk_wait_for_capabilities_entry(int port)
{
    ...
    /* SW ADD */
    //enable_not_auto_reply_goodcrc(port);
#endif /* #if (CONFIG_SUPPORT_SNK == 1) */
}
...
#endif
#if (OPT_SRC_ONLY == 1) || (OPT_SNK_ONLY)
        PE_CLR_DPM_REQUEST(port, DPM_REQUEST_PR_SWAP);
#endif
...
static void pe_snk_ready_entry(int port)
{
    ...
    /* SW ADD for notification UTCPD event {*/
    UTCPD_NotifyEvent(port, UTCPD_PD_SNK_VOLTAGE, pe[port].supply_voltage);
    /* SW ADD for notification UTCPD event }*/

#if (CONFIG_TC8260_FRS == 1)
    pd_timer_enable(port, TC_TIMER_FRS_START_DELAY,
                    PD_T_FRS_START_DELAY);
#endif
#endif /* #if (CONFIG_SUPPORT_SNK == 1)  */
}

static uint32_t snk_reay_count = 0;
...
static void pe_snk_ready_run(int port)
{
    ...
    /* FRS detect will be enabled after TC_TIMER_FRS_START_DELAY */
    if (pd_timer_is_disabled(port, TC_TIMER_FRS_START_DELAY) ||
            pd_timer_is_expired(port, TC_TIMER_FRS_START_DELAY))
    {
        pd_timer_disable(port, TC_TIMER_FRS_START_DELAY);
        PE_SET_FLAG2(port, PE_FLAG2S_FRS_DETECT);
    }
    ...
    if (PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
    {
        ...
        else
        {
            switch (type)
            {
                ...
                case PD_CTRL_NOT_SUPPORTED:
                    DBG_PRINTF("PD_CTRL_NOT_SUPPORTED\n");
                    /* Do nothing */
                    break;
...
#ifdef SW  /* Why snk role to discover CBL ? */
...
static void pe_snk_ready_exit(int port)
{
    ...
    pd_timer_disable(port, TC_TIMER_FRS_START_DELAY);
    PE_CLR_FLAG2(port, PE_FLAG2S_FRS_DETECT);        /* Disable FRS detect */
#endif  /* #if (CONFIG_SUPPORT_SNK == 1)  */
}
#if (CONFIG_TC8260_FRS == 1)
bool pe_is_enable_frs_detect(int port)
{
    return  PE_CHK_FLAG2(port, PE_FLAG2S_FRS_DETECT);
}
#endif
/**
 * PE_SNK_Hard_Reset
...
#endif /* #if (CONFIG_SUPPORT_DR_SWAP == 1) */
}
...
#endif /* #if (CONFIG_SUPPORT_PR_SWAP == 1) */
}
 ...
 * PE_FRS_SRC_SNK_Evaluate_Swap
 */
static void pe_frs_src_snk_evaluate_swap_entry(int port)
{
#if (CONFIG_SUPPORT_PR_SWAP == 1)
    print_current_state(port, __FUNCTION__);

    //if (!pd_check_power_swap(port) ||
    //      !PE_CHK_DPM_REQUEST(port, DPM_REQUEST_FR_SWAP) ) {
    if ((pd_check_power_swap(port) == 0) ||
            (PE_CHK_DPM_REQUEST(port, DPM_REQUEST_FR_SWAP) == DPM_REQUEST_FR_SWAP))
    {

        DBG_PRINTF("@@send hard reset requext\n");
        //send_ctrl_msg(port, TCPCI_MSG_SOP, PD_CTRL_REJECT);   //Reject if PR_SWAP
        pd_dpm_request(port, DPM_REQUEST_HARD_RESET_SEND);      //Hard Reset if we didn't support FR_SWAP message or we didn't initial FRS signal
    }
    else
    {
        tc_request_power_swap(port);
    ...
    PE_CLR_DPM_REQUEST(port, DPM_REQUEST_FR_SWAP);
#endif /* #if (CONFIG_SUPPORT_PR_SWAP == 1) */
}

static void pe_frs_src_snk_evaluate_swap_run(int port)
{
#if (CONFIG_SUPPORT_PR_SWAP == 1)
...
static void pe_prs_src_snk_transition_to_off_entry(int port)
{
    ...
    tcpc_addr_read16(port, 0, TCPC_REG_ROLE_CTRL, &u16Val);
    DBG_PRINTF("REG_ROLE_CTRL = 0x%x\n",  u16Val);
    tcpc_addr_read16(port, 0, TCPC_REG_CC_STATUS, &u16Val);
    DBG_PRINTF("REG_CC_STATUS = 0x%x\n",  u16Val);
...
#else
    if (pd_timer_is_expired(port, PE_TIMER_PS_SOURCE)) //DEBUG_ONLY, Ignore check VBUS
#endif
...
static void pe_prs_src_snk_wait_source_on_run(int port)
{
    ...
    {
        PE_CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
        DBG_PRINTF("Time out Error Recovery\n");
        set_state_pe(port, PE_WAIT_FOR_ERROR_RECOVERY);
        return;
    }
...
static void pe_prs_snk_src_assert_rp_run(int port)
{
    ...
    {
        DBG_PRINTF("attached src port\n");

        if (!IS_ENABLED(CONFIG_USB_PD_REV30) ||
                !pe_in_frs_mode(port))
        {
            /* Contract is invalid now */
            DBG_PRINTF("Contract is invalid\n");
            pe_invalidate_explicit_contract(port);
        }

        DBG_PRINTF("Next - turn on source\n");
        set_state_pe(port, PE_PRS_SNK_SRC_SOURCE_ON);
    }
    else
    {
        DBG_PRINTF("Still not src port\n");
    }
...
static void pe_prs_snk_src_source_on_run(int port)
{
    ...
    if (!pd_timer_is_disabled(port, PE_TIMER_PS_SOURCE))
    {
        DBG_PRINTF("Wait O SNK PS_SOURCE Time at least\n");

        if (!pd_timer_is_expired(port, PE_TIMER_PS_SOURCE))
        {
            DBG_PRINTF("Wait PS_SOURCE Time Out\n");
            return;
        }

#endif
        DBG_PRINTF("Send PD_CTRL_PS_RDY Msg\n");
        /* update pe power role */
        pe[port].power_role = pd_get_power_role(port);
        send_ctrl_msg(port, TCPCI_MSG_SOP, PD_CTRL_PS_RDY);
    ...
    {
        PE_CLR_FLAG(port, PE_FLAGS_TX_COMPLETE);
        DBG_PRINTF("Sent PD_CTRL_PS_RDY Msg done\n");
...
static void pe_bist_tx_entry(int port)
{
    ...
    if (mode == BIST_CARRIER_MODE_2)
    {
         ...
         * continuous string of BMC-encoded alternating "1"s and ???s.
...
#if 0 /* Remove it to avoid issue out hard reset after BIST pattern end (Tester didn't replied GoodCRC) */
        ...
        pe_set_ready_state(port);
#endif
        DBG_PRINTF("BIST carrier mode 2\n");
    }
...
static void pe_snk_give_sink_cap_entry(int port)
{
...
#if 0
    {
        //sw add
        uint32_t i;
        DBG_PRINTF("SNK PDO Count = %d\n", pd_snk_pdo_cnt);

        for (i = 0; i < pd_snk_pdo_cnt; i = i + 1)
        {
            uint32_t *pu32Ptr;
            //pu32Ptr = &tx_emsg[port].buf+i*4;
            pu32Ptr = &pd_snk_pdo[i];
            DBG_PRINTF("%d: 0x%x\n", i, *pu32Ptr);
        }
    }//sw add
#endif
    ...
    switch (parse_vdm_response_common(port))
    {
        case VDM_RESULT_WAITING:
            ...
            if (PE_CHK_FLAG(port, PE_FLAGS_PROTOCOL_ERROR))
            {
                ...
                PE_CLR_FLAG(port, PE_FLAGS_PROTOCOL_ERROR);
                set_state_pe(port, get_last_state_pe(port));
                VDM_PRINTF("VDM Error\n");
            }
            else
                VDM_PRINTF("VDM Waitting\n");

            return;

        case VDM_RESULT_NO_ACTION:
            DBG_PRINTF("VDM No Action\n");
...
static void pe_vdm_identity_request_cbl_exit(int port)
{
     ...
     * Not send any further SOP??SOP?™â€?Messages.
     */
...
{
#if (CONFIG_VDM_IDENTITY_REQUEST  == 1)
    uint32_t *msg = (uint32_t *)tx_emsg[port].buf;
     ...
     */
    pe[port].vdm_ack_min_data_objects = 4;
#endif /* #if (CONFIG_VDM_IDENTITY_REQUEST  == 1) */
}
...
#endif /* #if (CONFIG_VDM_SVIDS_REQUEST  == 1) */
}
...
static void pe_init_vdm_modes_request_entry(int port)
{
     ...
     */
    pe[port].vdm_ack_min_data_objects = 2;
#endif /* #if (CONFIG_VDM_MODES_REQUEST  == 1) */
}
...
#endif /* #if (CONFIG_VDM_REQUEST_DPM  == 1) */
}
...
{
#if (CONFIG_VDM_IDENTITY_REQUEST  == 1)  ||   (CONFIG_VDM_SVIDS_REQUEST  == 1) ||\
    (CONFIG_VDM_MODES_REQUEST  == 1) || (CONFIG_VDM_REQUEST_DPM  == 1)
    int vdo_len = 0;
    uint32_t *rx_payload;
    uint32_t *tx_payload;
    uint8_t vdo_cmd;
    ...
    switch (vdo_cmd)
    {
        case CMD_DISCOVER_IDENT:
            VDM_PRINTF("Discover ID\n");
            func = svdm_rsp.identity;
            break;

        case CMD_DISCOVER_SVID:
            VDM_PRINTF("Discover SVID\n");
            func = svdm_rsp.svids;
            break;

        case CMD_DISCOVER_MODES:
            VDM_PRINTF("Discover MODE\n");
            func = svdm_rsp.modes;
            break;

        case CMD_ENTER_MODE:
            VDM_PRINTF("Enter Mode\n");
        ...
        case CMD_EXIT_MODE:
            VDM_PRINTF("Exit MODE\n");
            func = svdm_rsp.exit_mode;
            break;
            //#ifdef CONFIG_USB_PD_ALT_MODE_DFP
#if (CONFIG_USB_PD_ALT_MODE_DFP == 1)
...
#endif   /* #if (CONFIG_VDM_IDENTITY_REQUEST  == 1)  ||   (CONFIG_VDM_SVIDS_REQUEST  == 1) ||
    (CONFIG_VDM_MODES_REQUEST  == 1) || (CONFIG_VDM_REQUEST_DPM  == 1) */
            ...
            PE_CHK_FLAG(port, PE_FLAGS_MSG_RECEIVED))
    {
        ...
        if (cnt == 0)
        {
            ...
            if (type == PD_CTRL_ACCEPT)
            {
                if (tc_is_vconn_src(port))
                {
                    DBG_PRINTF("\nVCONN SRC, ==> wait PS_Ready\n");
                    set_state_pe(port,
                                 PE_VCS_WAIT_FOR_VCONN_SWAP);
                }
                else
                {
                    DBG_PRINTF("\nVCONN SNK, ==> Turn-n VCONN\n");
                    set_state_pe(port,
                                 PE_VCS_TURN_ON_VCONN_SWAP);
                }
...
static void pe_vcs_wait_for_vconn_swap_run(int port)
{
    ...
    if (pd_timer_is_expired(port, PE_TIMER_VCONN_ON))
    {
        if (pe[port].power_role == PD_ROLE_SOURCE)
        {
            DBG_PRINTF("VCONN SRC HR\n");
            printf("L %d\n", __LINE__);
            set_state_pe(port, PE_SRC_HARD_RESET);
        }
        else
        {
            DBG_PRINTF("VCONN SNK HR\n");
            printf("L %d\n", __LINE__);
            set_state_pe(port, PE_SNK_HARD_RESET);
        }
    }
        ...
        if (ext == 0 && sop == TCPCI_MSG_SOP)
        {
            if ((cnt > 0) && (type == PD_DATA_SINK_CAP))
            {
...

                {
                    //sw add
                    uint32_t i;
                    DBG_PRINTF("REC SNK PDO Count = %d\n", cap_cnt);
        ...
        if (ext == 0)
        {
            if ((cnt > 0) && (type == PD_DATA_SOURCE_CAP))
            {
...

                {
                    //sw add
                    uint32_t i;
                    DBG_PRINTF("REC SRC PDO Count = %d\n", cnt);
...
#ifdef SW  /* We still keep as SRC, not to do PR_SWAP */
...
{
    DBG_PRINTF("DISCOVERY_TYPE_COUNT = %d\n", DISCOVERY_TYPE_COUNT);
    DBG_PRINTF("TCPCI_MSG_SOP = %d\n", TCPCI_MSG_SOP);
    DBG_PRINTF("TCPCI_MSG_SOP_PRIME = %d\n", TCPCI_MSG_SOP_PRIME);
...
static __const_data const struct usb_state pe_states[] =
{
    ...
    /* SW Add For SRC to SNK Fast Role Swap */
    [PE_FRS_SRC_SNK_EVALUATE_SWAP] = {
        .entry = pe_frs_src_snk_evaluate_swap_entry,  //.entry = pe_prs_src_snk_evaluate_swap_entry,
        .run   = pe_frs_src_snk_evaluate_swap_run,    //.run   = pe_prs_src_snk_evaluate_swap_run,
    },
...
void UTCPD_PE_get_src_caps(int port, int32_t *pu32SrcArray, int32_t *pi32SrcCnt)
{
    ...
    for (i = 0; i < pe[port].src_cap_cnt; i = i + 1)
        pu32SrcArray[i] = pe[port].src_caps[i];

    *pi32SrcCnt = pe[port].src_cap_cnt;
}
...
void UTCPD_PE_get_snk_caps(int port, int32_t *pu32SnkArray, int32_t *pi32SnkCnt)
{
    ...
    for (i = 0; i < pe[port].snk_cap_cnt; i = i + 1)
        pu32SnkArray[i] = pe[port].snk_caps[i];

    *pi32SnkCnt = pe[port].snk_cap_cnt;
}
