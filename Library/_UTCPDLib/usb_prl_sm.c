...
#if (CONFIG_USB_PD_REV30 == 0)//2022/04/01
...
void pd_execute_hard_reset(int port)
{
    ...
    // SW Add {
    task_clear_event(port, PD_EVENT_RX_HARD_RESET);
    // SW Add }
...
{
    int i;
    const struct sm_ctx cleared = {NULL};
...
#if 0
    /* SW ADD */
    outp32(UTCPD0_BASE + TCPC_REG_MSG_HDR_INFO, (inp32(UTCPD0_BASE + TCPC_REG_MSG_HDR_INFO) & ~0x06) | (rev << 1));
#else
    //outp32(UTCPD0_BASE+TCPC_REG_MSG_HDR_INFO, (inp32(UTCPD0_BASE+TCPC_REG_MSG_HDR_INFO) & ~0x06) | (PD_REV20 <<1));
#endif
}
...
void prl_copy_msg_to_buffer_swex(int port)
{
    prl_copy_msg_to_buffer(port);
}
...
void increment_msgid_counter_swex(int port)
{
...
/* SW Add fpr operation create command directly */
void prl_tx_construct_message_swex(int port)
{
    prl_tx_construct_message(port);
}
...
void prl_hr_send_msg_to_phy_swex(int port)
{
    prl_hr_send_msg_to_phy(port);
}


/* Cable Reset Operation (Clone from prl_hr_send_msg_to_phy)*/
void prl_cr_send_msg_to_phy_swex(const int port)
{
    /* Header is not used for cable reset */
    const uint32_t header = 0;

    pdmsg[port].xmit_type = TCPCI_MSG_CABLE_RESET;
...
/* Cable Reset Operation (Clone from prl_hr_send_msg_to_phy)*/
void prl_bist_send_msg_to_phy_swex(const int port)
{
    /* Header is not used for cable reset */
    const uint32_t header = 0;

    pdmsg[port].xmit_type = TCPCI_MSG_TX_BIST_MODE_2;
...
static void prl_hr_reset_layer_run(const int port)
{
    ...
    if (PRL_HR_CHK_FLAG(port, PRL_FLAGS_PE_HARD_RESET))
    {
        ...
        /* SW Add {*/
        PRL_HR_CLR_FLAG(port, PRL_FLAGS_PE_HARD_RESET);
        /* SW Add }*/
    }
    /*
     * Protocol Layer reset complete &
...
#if 0
void copy_chunk_to_ext_swex(int port)
{
    uint32_t u32PdoCnt, i;
    uint32_t header = inp32(UTCPD0_BASE + TCPC_REG_RX_HDR);
    pdmsg[port].num_bytes_received = PD_HEADER_CNT(header) * 4;
    u32PdoCnt = PD_HEADER_CNT(header);

    for (i = 0; i < u32PdoCnt; i = i + 1)
    {
        pdmsg[port].rx_chk_buf[i] = inp32(UTCPD0_BASE + TCPC_REG_RX_DATA + i * 4);
    }
