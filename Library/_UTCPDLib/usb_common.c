...
int pd_check_requested_voltage(uint32_t rdo, const int port)
{
    ...
    {
        printf("RDO_CAP_MISMATCH\n");
        return EC_ERROR_INVAL; /* too much max current */
    }

    //  printf("======\n");
    //  printf("src pdo cnt = 0x%x\n",pd_src_pdo_cnt);
    //  printf("rdo = 0x%x\n", rdo);
    //  printf("idx = 0x%x\n", idx);
    //  printf("pdo = 0x%x\n", pdo);
    //  printf("Requested %d mV %d mA (for %d/%d mA)\n",
...
/* defined in board level */
#define PD_OPERATING_POWER_MW       15000
#define PD_MAX_CURRENT_MA           3100 //3000
#define PD_MAX_VOLTAGE_MV           21000 //20000

/* ---------------- Power Data Objects (PDOs) ----------------- */
#if (CONFIG_USB_PD_CUSTOM_PDO == 0)
...
#ifdef SW_1_0   /* It can defined  in board level's usb_pd_poicy.c */
...
#else
    return tcpm_check_vbus_level(port, level);    /* TC8260 get VBUS from TCPC */
#endif
}
...
/*
return 1 means : No interrupt channel enabled
return 0 means : Someone interrupt channel enabled
*/
int32_t disable_irq(void)
{
    uint32_t i;
    int32_t ismask = 1;
    ...
    {
        /* Backup */
        NVIC_ISER[0] = NVIC->ISER[0];
        NVIC_ISER[1] = NVIC->ISER[1];
        NVIC_ISER[2] = NVIC->ISER[2];
        NVIC_ISER[3] = NVIC->ISER[3];
        NVIC_ISER[4] = NVIC->ISER[4];
...
{
    NVIC->ISER[0] = NVIC_ISER[0];
    NVIC->ISER[1] = NVIC_ISER[1];
    NVIC->ISER[2] = NVIC_ISER[2];
    NVIC->ISER[3] = NVIC_ISER[3];
    NVIC->ISER[4] = NVIC_ISER[4];
}
#endif
#endif
#endif
