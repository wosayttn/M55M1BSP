...
static int nct38xx_init(int port)
{
...
#else
    DBG_PRINTF("Unknown why 3806 init to enable SNKEN and SRCEN\n");
#endif
...
#if 0    /* Same as NCT3806 Pin Polarity Control Register */
    ...
    rv = tcpc_update8(port,
                      ...
                      MASK_SET);
#else
    DBG_PRINTF("Please enable VCONN OCP and OVP hear\n");
#endif
    return rv;
}
...
static int nct38xx_tcpm_set_cc(int port, int pull)
{
     ...
     */
    int rv;
    DBG_PRINTF("%s pull1= %x\n", __FUNCTION__, pull);
    enum mask_update_action action =
        pull == TYPEC_CC_OPEN && tcpm_get_snk_ctrl(port) ?
        MASK_CLR : MASK_SET;
    DBG_PRINTF("%s pull2= 0x%x, action = 0x%x\n", __FUNCTION__, pull, action);
#ifdef SW     /* TC8260 without the register */
    rv = tcpc_update8(port,
...
#else
    DBG_PRINTF("Unknown why 3806 enable SNKEN\n");
#endif
...
#else
        DBG_PRINTF("Please enable SNK MOSFET to SNK VBUS\n");
#endif
    }
...
#else
#if (CONFIG_IO_EXPANDER_NCT38XX == 1)
