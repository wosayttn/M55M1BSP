...
/* SW ADD{ */
/* should be removed after ./common/usb_common.c or ./board/board  */
...
#else
    #if defined(DEBUG_I2C_FAULT_LAST_WRITE_OP)
        struct i2c_wrt_op last_write_op[CONFIG_USB_PD_PORT_MAX_COUNT];
    #endif
#endif
/*
...
{
    int rv;
#ifdef SW  /* Removed directly due to not suppoty LPM */
                      ...
                      enum tcpc_cc_voltage_status *cc2)
{
    ...
    *cc2 |= cc2_present_rd << 2;
            ...
            last_get_cc[port].role != role))
    {
        ...
        //        printf("C%d: GET_CC cc1=%d cc2=%d cc_sts=0x%X role=0x%X",
        //            port, *cc1, *cc2, status, role);
...
#else
#if (DEBUG_ROLE_CTRL_UPDATES == 1)
    DBG_PRINTF("C%d: SET_CC pull=%d role=0x%x\n", port, pull, role);
#endif
#endif
...
#else
#if (DEBUG_ROLE_CTRL_UPDATES == 1)
    printf("C%d: SET_ROLE_CTRL drp=%d rp=%d pull=%d role=0x%X",
           port, drp, rp, pull, role);
#endif
#endif
...
bool tcpci_tcpm_get_snk_ctrl(int port)
{
    ...
    DBG_PRINTF("%s - pwr_sts\n", __FUNCTION__, pwr_sts);
...
bool tcpci_tcpm_check_vbus_level(int port, enum vbus_level level)
{
    ...
    if (level == VBUS_SAFE0V)
    {
        i32Ret = !!(tcpc_vbus[port] & BIT(VBUS_SAFE0V));
        //printf("#CHK VSEFE0V = 0x%x\n", i32Ret);
        return i32Ret;
    }
    else if (level == VBUS_PRESENT)
    {
        i32Ret = !!(tcpc_vbus[port] & BIT(VBUS_PRESENT));
        //DBG_PRINTF("#CHK VPresent = 0x%x\n", i32Ret);
        return i32Ret;
    }
    else
    {
...
#ifdef SW /* seems only log message */
...
#ifdef SW        //UTCPD has not the register    
...
int tcpci_get_chip_info(int port, int live, struct ec_response_pd_chip_info_v1 *chip_info)
{
    static struct ec_response_pd_chip_info_v1 cached_info[CONFIG_USB_PD_PORT_MAX_COUNT];
...
#if (CONFIG_USB_PD_DISCHARGE_TCPC == 1)
