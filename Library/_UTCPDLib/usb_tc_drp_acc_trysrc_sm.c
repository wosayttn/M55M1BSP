...
//#define PD_POWER_SUPPLY_TURN_ON_DELAY            (30000) /* us. It should be define in board level */
...
#define assert(x)      printf("tc assert(%d)\n", x);

/* Modify from ./common/battery.c and include/battery.h */
...
/* Copy from ./common/usb_common.c */
bool board_is_dts_port(int port)
{
 return true;
}
/* SW Add } */
...
#define CPRINTS_L1 printf
#define CPRINTS_L2 printf
#define CPRINTS_L3 printf  
 ...
 */
#define CONFIG_USB_PD_TCPC_LPM_EXIT_DEBOUNCE     (100 * MSEC)   /* SW add after search and compare */
...
#else
static volatile 
enum pd_dual_role_states drp_state[CONFIG_USB_PD_PORT_MAX_COUNT] = {CONFIG_USB_PD_INITIAL_DRP_STATE};
#endif     
//static void set_vconn(int port, int enable);
void set_vconn(int port, int enable);
...
__overridable enum pd_dual_role_states board_tc_get_initial_drp_mode(int port)
{
  ...
  return PD_DRP_FORCE_SOURCE;    /* The state keep in SRC role, and didn't toggle */
#endif   
...
//#if 0
///* SW copy from ./common/usb_pd_dual_role.c { */
...
static bool pd_comm_allowed_by_policy(void)
{
  ...
  return false;
#else
  pd_disabled_on_init = 1;    /* SW ??? */
  return true;  /* Always enable true */
#endif  
}
...
#else
  //printf("Please implement PD connect/disconnect notify\n");
...
static bool tc_perform_snk_hard_reset(int port)
{
  ...
  case PS_STATE0:
    ...
    return false;
  case PS_STATE1:
    DBG_PRINTF("SNK HR S1\n");
#if 0  //DEBUG_ONLY
    if (pd_check_vbus_level(port, VBUS_SAFE0V)) { 
#else
    if (1) {//Assume VBUS is 0V  
#endif   
      /*
    ...
    if (pd_timer_is_expired(port, TC_TIMER_TIMEOUT)) {
      ...
      pd_timer_enable(port, TC_TIMER_TIMEOUT,
          PD_T_SRC_RECOVER_MAX +
          PD_T_SRC_TURN_ON);
      DBG_PRINTF("VBUS 0V Tout\n");
    }
    return false;
  case PS_STATE2:
    DBG_PRINTF("SNK HR S2\n");
    /*
...
static void restart_tc_sm(int port, enum usb_tc_state start_state)
{
  ...
  res = tcpm_init(port);
    ...
    /** For FRS setting 
      ...
      * Use PC5 FRX_TX2 */
#if 1
    SYS->GPC_MFP1 = (SYS->GPC_MFP1 & ~(0xFFUL << (0*8))) | (17UL << (0*8)); //PC4 FRS_TX1
    SYS->GPC_MFP1 = (SYS->GPC_MFP1 & ~(0xFFUL << (1*8))) | (17UL << (1*8)); //PC5 FRS_TX2
    frs_mux_selection(1, 1);

    /* Enable Alert[Vender] */ 
    /* Enable Vendor define Interrupt */
    outp32(UTCPD0_BASE + TCPC_REG_ALERT_MASK, inp32(UTCPD0_BASE + TCPC_REG_ALERT_MASK)| TCPC_REG_ALERT_MASK_VENDOR_DEF);    
    outp32(UTCPD0_BASE + UTCPD_VNDIE, inp32(UTCPD0_BASE + UTCPD_VNDIE)| (RXFRSIE|TXFRSIE|CRCERRIE|VCDGIE));
    outp32(UTCPD0_BASE + UTCPD_FILTM, 1);
#endif

  CPRINTS("C%d: TCPC init %s", port, res ? "failed" : "ready");
  DBG_PRINTF("C%d: TCPC init %s", port, res ? "failed" : "ready");
...
void tc_state_init(int port)
{
  ...
  first_state = TC_UNATTACHED_SNK;       /* To turn off VBUS and VCONN */
#endif
#if (CONFIG_SUPPORT_SRC==1)
  first_state = TC_UNATTACHED_SRC;       
#endif
  //first_state = TC_ERROR_RECOVERY;
#endif
...
void tc_event_check(int port, int evt)
{
...
#ifdef SW  
  if (evt & PD_EVENT_TCPC_RESET)  /* PD_EVENT_TCPC_RESET flag is set in LOW Power mode, We didn't support it */
  ...
  {
    //DBG_PRINTF("evt = PD_EVENT_UPDATE_DUAL_ROLE\n");
...
static void pd_update_try_source(void)
{
  ...
  #ifdef SW
  tc_enable_try_src(pd_is_try_source_capable());    /* SW defined in common\usb_pd_dual_role.c */
  #else
  tc_enable_try_src(CONFIG_USB_PD_TRY_SRC);
  #endif
#endif
}
//DECLARE_HOOK(HOOK_BATTERY_SOC_CHANGE, pd_update_try_source, HOOK_PRIO_DEFAULT);

bool bIsDetectEMarker = false; 
...
void set_vconn(int port, int enable)
{
  ...
  else
  {
    ...
    if(enable == true)
      printf("VCONN 1 ==> But disabled\n");
    else
      printf("VCONN 0 ==> disabled\n");
...
/* SW Add { */
void det_emarker(port)
{
    int status;
    enum tcpc_cc_voltage_status cc1;
    enum tcpc_cc_voltage_status  cc2;
    
    tcpc_read(port, TCPC_REG_CC_STATUS, &status);
    
    /* errors will return CC as open */
    cc1 = TYPEC_CC_VOLT_OPEN;
    cc2 = TYPEC_CC_VOLT_OPEN;
    
    printf("cc_status = 0x%x\n", status); 
    cc1 = TCPC_REG_CC_STATUS_CC1(status);
    cc2 = TCPC_REG_CC_STATUS_CC2(status);
    if( (cc1 == TYPEC_CC_VOLT_RA) || (cc2 == TYPEC_CC_VOLT_RA))
    {
        bIsDetectEMarker = true;
        printf("Det-Ra, %s VC on\n", __FUNCTION__);
        set_vconn(port, 1);
    }
    else
    {    
        printf("Non-Ra, %s VC off\n", __FUNCTION__);
        bIsDetectEMarker = false;
        set_vconn(port, 0);
    }
}
/* SW Add } */
...
__maybe_unused static void handle_new_power_state(int port)
{
#ifdef SW  /* To avoid the chipset_in_or_transitioning_to_state() call */
  if (!IS_ENABLED(CONFIG_POWER_COMMON))
    assert(1);
...
#ifdef SW /* The function only to be called in Low Power Mode. We didn't support it */
static __maybe_unused int reset_device_and_notify(int port)
{
  int rv;
...
{
#ifdef SW /* TC8260 didn't support LOW_POWER Mode */
...
static void tc_unattached_snk_entry(const int port)
{
  ...
  UTCPD_NotifyEvent(port, UTCPD_PD_ATTACHED, 0); 
  /* SW ADD for notification UTCPD event }*/
...
static void tc_attach_wait_snk_run(const int port)
{
...
#if 1   /* SW Remove DEBUG_ONLY: For Galaxy Lite can works well */
  ...
  VBUS_Sink_Enable(port, 1);      //pd_set_input_current_limit(0, 500, 5000)
...
#else
    DBG_PRINTF("Please implement PD connect/disconnect notify\n");
#endif  
...
#ifdef SW  /* NCT3806 didn't support Debug Accessory Mode */  
    tcpm_debug_accessory(port, 1);
    set_ccd_mode(port, 1);
#else
      DBG_PRINTF("TC8260 didn't support Debug Accessory Mode\n");
#endif    
  }
  
  /* SW ADD for notification UTCPD event {*/
  UTCPD_NotifyEvent(port, UTCPD_PD_ATTACHED, 1); 
  /* SW ADD for notification UTCPD event }*/
...
static void tc_attached_snk_run(const int port)
{
  ...
  if (tc_get_pd_enabled(port) && prl_is_running(port)) {
    ...
    if (TC_CHK_FLAG(port, TC_FLAGS_REQUEST_PR_SWAP)) {
      DBG_PRINTF("@@@ SNK RUN: CHK TC_FLAGS_REQUEST_PR_SWAP");
      /*
...
static void tc_unattached_src_entry(const int port)
{
  ...
  if (get_last_state_tc(port) != TC_UNATTACHED_SNK) {
    DBG_PRINTF("Last state != TC_UNATTACHED_SNK\n");
    tc_detached(port);    print_current_state(port, __FUNCTION__);
  }
  ...
  /* SW ADD start discharge */
  /* To avoid the VCONN discharge with wrong voltage on CC pin */
  /* It will cause DRP with wrong result */
  //vconn_enable_discharge();    ==> Move to "tc_attached_src_exit"
...
static void tc_attach_wait_src_run(const int port)
{
  ...
  if (cc_is_snk_dbg_acc(cc1, cc2) && board_is_dts_port(port)) {
    ...
    {
      set_state_tc(port, TC_UNATTACHED_SRC);
      DBG_PRINTF("No UFP: Still Force SOURCE\n");  
    }
    else
    {
      set_state_tc(port, TC_UNATTACHED_SNK);
      DBG_PRINTF("No UFP: Going to TC_UNATTACHED_SNK\n");  
    }
    return;
  }
...
//#endif      
      ...
      if (IS_ENABLED(CONFIG_USBC_VCONN) &&
          ...
          set_vconn(port, 1);
#else      
      {/* To pass Type-C Functional Tests TD4.6.5 and TD 4.8.3 */
          /* SW ADD : Detect Ra first */
          det_emarker(port);    
          set_vconn(port, 1);  
          /* SW ADD */
      }
...
static void tc_attached_src_run(const int port)
{
  ...
  if (tc_get_pd_enabled(port) && prl_is_running(port)) {
     ...
     */
    if (TC_CHK_FLAG(port, TC_FLAGS_REQUEST_PR_SWAP)) {
      DBG_PRINTF("@@@ SRC RUN: CHK TC_FLAGS_REQUEST_PR_SWAP");
...
#ifdef SW  /* Board layer adjust the current capacability */
...
static void tc_attached_src_exit(const int port)
{
  ...
  if (!TC_CHK_FLAG(port, TC_FLAGS_REQUEST_PR_SWAP)) {
    ...
    #else
    /* SW ADD start discharge                     */
    /* To avoid the VCONN discharge time too long */
    /* It will cause DRP with wrong result        */
    if (TC_CHK_FLAG(port, TC_FLAGS_VCONN_ON) &&
        !TC_CHK_FLAG(port, TC_FLAGS_CTVPD_DETECTED))
    {
//      typec_select_pull(port, TYPEC_CC_RD);  /* Test to speed up discharge time */
//      typec_update_cc(port);
      printf("Dis VCONN and DC\n");
      if( inp32(UTCPD0_BASE+TCPC_REG_TCPC_CTRL) & TCPC_REG_TCPC_CTRL_PLUG_ORIENTATION )
          outp32(UTCPD0_BASE+TCPC_REG_ROLE_CTRL, inp32(UTCPD0_BASE+TCPC_REG_ROLE_CTRL) | TCPC_REG_ROLE_CTRL_CC1_MASK);  //CC2 as CC, Open CC1
      else
          outp32(UTCPD0_BASE+TCPC_REG_ROLE_CTRL, inp32(UTCPD0_BASE+TCPC_REG_ROLE_CTRL) | TCPC_REG_ROLE_CTRL_CC2_MASK);  //CC1 as CC, Open CC2
      set_vconn(port, 0);
    #if 0  
      vconn_enable_discharge();  
            delay(200);      
      vconn_disable_discharge();  
    #else
      PA1 = 1;   /* Start Discharge */
      delay(500);
      PA1 = 0;  /* Stop Discharge */
    #endif    
    }
    #endif
  }
...
//#ifdef CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE
  ...
  case DRP_TC_DRP_AUTO_TOGGLE:
    set_state_tc(port, TC_DRP_AUTO_TOGGLE);
    DBG_PRINTF("Next State = TC_DRP_AUTO_TOGGLE\n");    
    break;
#endif
...
void check_drp_connection_swex(const int port)
{
...
__maybe_unused static void tc_drp_auto_toggle_entry(const int port)
{
  ...
  /* SW ADD */
  /* For plug out then plug, DRP auto toggling is fail if enable VCONN */
  /* Before issue out "TCPC_REG_COMMAND_LOOK4CONNECTION" command. the VCONN voltage should be discharge to close < 200mv */
  ...
  /* We didn't increase the code due to nct3806.c implement tcpc_config[port].drv->drp_toggle(port)
     It will call tcpci_tcpc_drp_toggle() */
#endif /* #if (CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE == 1) */
}
...
__maybe_unused static void tc_drp_auto_toggle_run(const int port)
{
  ...
  if (TC_CHK_FLAG(port, TC_FLAGS_CHECK_CONNECTION))
  {
    ...
    printf("tcpm_enable_drp_toggle\n");
    pd_timer_disable(port, TC_TIMER_TIMEOUT);    
    tcpm_enable_drp_toggle(port);

#if 0  
    DBG_PRINTF("Hardware auto Toggle, Unnecessary software to do toggle\n");
    {//SW Add for next toggling
        pd_timer_enable(port, TC_TIMER_TIMEOUT,
            MAX(PD_T_DRP_SNK, PD_T_DRP_SRC));
    }
#endif    
...
#if (CONFIG_USB_PD_TCPC_LOW_POWER == 1) /* SW ADD { */
...
static void tc_try_src_run(const int port)
{
  ...
  if(new_cc_state == PD_CC_UFP_ATTACHED)
    DBG_PRINTF("cc state = PD_CC_UFP_ATTACHED\n");
  else if(new_cc_state == PD_CC_NONE)
    DBG_PRINTF("cc state = PD_CC_NONE\n");
  /* Debounce the cc state */
  if (new_cc_state != tc[port].cc_state) {
    tc[port].cc_state = new_cc_state;
    pd_timer_enable(port, TC_TIMER_CC_DEBOUNCE, PD_T_CC_DEBOUNCE);
...
#endif /* SW_UNKNOWN. callback? what application will defered these API ? */
