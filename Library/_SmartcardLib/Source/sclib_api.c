...
int32_t SCLIB_Activate(uint32_t num, uint32_t u32EMVCheck)
{
    ...
    if ((retval == SCLIB_SUCCESS) && (dev->CardCapabilities.Protocol.Selected == SCLIB_PROTOCOL_T1) && (dev->EMV == TRUE))
    {
        retval = SCLIB_SetIFSD(num, 0xFE);
    }
...
int32_t SCLIB_WarmReset(uint32_t num)
{
    ...
    if (dev->errno != SCLIB_SUCCESS && dev->errBypass == 0)
        return (dev->errno);

    dev->pps_complete = 0;
    dev->errno = _SCLIB_UpdateCardCapabilities(num);    // Hmmm, _SCLIB_ParseATR might call this after PPS exchange executed.
...
int32_t SCLIB_StartTransmission(uint32_t num, uint8_t *cmdBuf, uint32_t cmdLen, uint8_t *rspBuf, uint32_t *rspLen)
{
    ...
    if ((dev->CardCapabilities.Protocol.Selected & SCLIB_PROTOCOL_T0) == SCLIB_PROTOCOL_T0)
    {
        retval = _SCLIB_T0Transmit(num, cmdBuf, cmdLen, rspBuf, rspLen);
    }
    else if ((dev->CardCapabilities.Protocol.Selected & SCLIB_PROTOCOL_T1) == SCLIB_PROTOCOL_T1)
    {
...
/* The API is for Chip without Card detection pin */
uint32_t SCLIB_CheckCDEvent_ByVar(uint32_t num, volatile uint32_t *u32CardIsInserted)
{
    SCLIB_DEV_T *dev = &_scDev[num];
    SC_T *sc = _scBase[num];
    uint32_t retval = 0;
    static uint32_t u32CardDetection_previous = 0;       /*  static variable needs compiling time constant */

    if (u32CardDetection_previous != *u32CardIsInserted) /*  S/W card detection happen                     */
    {
        if (*u32CardIsInserted == 1)                     /*  Card is inserted                              */
        {
            SCLIB_INFO("SC%d: Card Inserted\n", num);
            dev->bCardRemoved = SCLIB_CARD_PRESENT;
        }
        else                                            /*  Card is removed                                */
        {
        ...
        retval = 1;
        u32CardDetection_previous = *u32CardIsInserted; //keep the card detection history.
    }
...
uint32_t SCLIB_CheckCDEvent(uint32_t num)
{
    ...
    if (sc->INTSTS & SC_INTSTS_CDIF_Msk)     /* toggle detect by card */
    {
        ...
        sc->INTSTS = SC_INTSTS_CDIF_Msk; // clear CD_IS bit
        reg = sc->STATUS;

        if ((reg & (SC_STATUS_CREMOVE_Msk | SC_STATUS_CINSERT_Msk)) ==
                (SC_STATUS_CREMOVE_Msk | SC_STATUS_CINSERT_Msk))    // CD_INS_F & CD_REM_F both trigger
        {
        ...
        {
            sc->STATUS |= SC_STATUS_CINSERT_Msk;   // clear CD_INS_F flag
            SCLIB_INFO("SC%d: Card Inserted\n", num);
            ...
            dev->bCardRemoved = SCLIB_CARD_PRESENT;
                 ...
                 (((reg & SC_STATUS_CDPINSTS_Msk) >> SC_STATUS_CDPINSTS_Pos) != ((sc->CTL & SC_CTL_CDLV_Msk) >> SC_CTL_CDLV_Pos)))
        {
            sc->STATUS |= SC_STATUS_CREMOVE_Msk;   // clear CD_REM_F flag
            dev->bCardRemoved = SCLIB_CARD_ABSENT;
            SCLIB_INFO("SC%d: Card Removed\n", num);
...
uint32_t SCLIB_CheckTxRxEvent(uint32_t num)
{
    ...
    if (sc->INTSTS & (SC_INTSTS_RDAIF_Msk | SC_INTSTS_RXTOIF_Msk))
    {
        ...
        else if (dev->op_state == SCLIB_OP_ATR_READ)     /* Read ATR ISR */
        {
            ...
            /* [EMV2000] */
            if (!dev->specificBR) SC_StartTimer(sc, 0, SC_TMR_MODE_0, 9600 + 480);
            else SC_StartTimer(sc, 0, SC_TMR_MODE_0, (SC_GetInterfaceClock(sc) * 1000 / dev->AtrEtu) + 480);
            ...
            if (atr_remain == 0)    /* receive ATR done */
            {
                ...
                SC_StopTimer(sc, 0);
                SC_StartTimer(sc, 0, SC_TMR_MODE_0, 480);  // Hmmm, what is this?
                SC_StopTimer(sc, 2);
                atr_final_chk = 1;
...
uint32_t SCLIB_CheckErrorEvent(uint32_t num)
{
    ...
    if ((sc->INTSTS & SC_INTSTS_TERRIF_Msk) && (sc->INTEN & SC_INTEN_TERRIEN_Msk))
    {
        ...
        if (sc->STATUS & SC_STATUS_TXOVERR_Msk)
        {
            if (dev->op_state == SCLIB_OP_WRITE)
                sc->INTEN &= ~SC_INTEN_TBEIEN_Msk;

            sc->STATUS = (SC_STATUS_TXRERR_Msk | SC_STATUS_TXOVERR_Msk);
        ...
        if (sc->STATUS & SC_STATUS_RXOVERR_Msk)
        {
            sc->STATUS = (SC_STATUS_RXRERR_Msk | SC_STATUS_RXOVERR_Msk);
            dev->errno = SCLIB_ERR_READ;
            dev->op_state = SCLIB_OP_IDLE;
...
int32_t SCLIB_SetSpecificAtrBR(uint32_t num, uint32_t br)
{
    ...
    etu = SC_GetInterfaceClock(sc) * 1000 / br;
