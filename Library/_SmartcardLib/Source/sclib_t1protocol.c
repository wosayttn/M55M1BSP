...
/** @addtogroup M55_Library M25 Library
  @{
*/
...
/**
  ...
  * @param  num Smartcard interface number. From 0 ~ ( \ref SC_INTERFACE_NUM - 1)
  * @return The result status of process.
  * @retval SCLIB_SUCCESS Success
  * @retval Others Failed.
  */
int _SCLIB_SendBlock(uint32_t num)
{
    ...
    /* [EMV 2000] */
    SC_StopTimer(sc, 0);
    SC_StartTimer(sc, 0, SC_TMR_MODE_7, (dev->CardCapabilities.T1.BWT / dev->CardCapabilities.etu) +
                  (BitRateAdjustment[dev->CardCapabilities.Dl].DNumerator * 2400) + 1);
...
int32_t _SCLIB_ProcessSBlock(uint32_t num)
{
    ...
    else if (recBlock[1] == SCLIB_T1_BLOCK_S_ABORT_REQ)
    {
        ...
        // No need to send ABORT response in EMV
        if (dev->EMV != TRUE)
            _SCLIB_SendSBlock(num, SCLIB_T1_BLOCK_S_ABORT_RES, 0);
    ...
    while (retVal != SCLIB_SUCCESS && retransmitTimes < SCLIB_MAX_T1_RETRANSMIT_CNT)
    {
        ...
        if (retVal != SCLIB_SUCCESS)
        {
            ...
            else
            {
                ...
                if (dev->EMV == TRUE)
                {
                    // No need to handle re-sync in EMV
                    return retVal;
                }
                else
                {
