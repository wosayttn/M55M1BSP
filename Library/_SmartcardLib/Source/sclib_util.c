...
static void _SCLIB_SetBaudrate(uint32_t num, int32_t fi, int32_t di)
{
    ...
    sc->ETUCTL = (ClockRateConversion[fi].F / BitRateAdjustment[di].DNumerator) - 1;
    ...
    else if (dev->CurrentState == SCLIB_CARD_NEGOTIABLE)        /* in negotiable mode */
    {
        ...
        if ((Fi != 0) || (Di != 1))
        {
            if (dev->EMV == TRUE)
            {
                ...
                atr = dev->CardCapabilities.ATR.Buffer;
                atr++;  // TS->T0
                atr++;  // T0->TA1
                *atr = 0x11; // Fi = Di = 1
                ...
                {
                    atr = dev->CardCapabilities.ATR.Buffer;
                    atr[dev->CardCapabilities.ATR.Length - 1] = 0x00;

                    for (i = 1; i < (dev->CardCapabilities.ATR.Length - 1); i++)
                        atr[dev->CardCapabilities.ATR.Length - 1] ^= atr[i];
                }

                dev->CardCapabilities.Protocol.Selected = SCLIB_PROTOCOL_T0;
...
void _SCLIB_StartColdReset(uint32_t num)
{
    ...
    if (!dev->specificBR) SC_StartTimer(sc, 0, SC_TMR_MODE_0, (42000 / 372) + 13);
    else SC_StartTimer(sc, 0, SC_TMR_MODE_0, (42000 / dev->AtrEtu) + 13);
...
int32_t _SCLIB_ResetCard(uint32_t num, uint32_t u32ResetType)
{
    ...
    if (!dev->specificBR) _SCLIB_SetBaudrate(num, 1, 1);       // default 1 ETU == 372 cycles
    else sc->ETUCTL = dev->AtrEtu - 1; // no pair avialable
