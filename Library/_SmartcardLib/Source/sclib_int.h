...
#define SCLIB_MAX_T1_BUFFER_SIZE        512
...
typedef struct
{
    ...
    uint32_t GT;  ///< Calculated guard time in micro seconds
...
typedef struct
{
    volatile uint32_t op_state;    /* 0 - everything goes well, 1 - some error occurred */
    volatile int32_t errno;
    uint32_t openflag;
    //uint32_t clock;                /* frequency in KHz */
    uint32_t EMV;           ///< Apply EMV check
