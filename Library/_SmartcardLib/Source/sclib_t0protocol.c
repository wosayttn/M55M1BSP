...
int32_t _SCLIB_T0Transmit(uint32_t num, uint8_t *cmdBuf, uint32_t cmdLen, uint8_t *rspBuf, uint32_t *rspLen)
{
    ...
    if (status == SCLIB_SUCCESS)
    {
        ...
        sc->INTEN &= ~SC_INTEN_TBEIEN_Msk;  // need to disable tx interrupt anyway. we're not sending data now... ya
        ...
        do
        {
            ...
            if (dev->T0.Lc != 0)
            {
                ...
                // keep snd_buf in user application, save some memcpy calls ; i don't understand this setting
