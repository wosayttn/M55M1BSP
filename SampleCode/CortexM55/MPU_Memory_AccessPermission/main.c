 ...
 * @file    main.c
 * @version V1.00
 * @brief   To configure memory region as read-only region
 *
...
void MPU_TestAccess(void)
{
     ...
     * Start address = TEST_REGION_1_BASE_ADDR
     * Shareablity   = Non-shareable
     * Size          = TEST_REGION_1_SIZE
     * Permission    = Full access
     */
    ARM_MPU_SetRegion(1UL,
                      ARM_MPU_RBAR(TEST_REGION_1_BASE_ADDR, ARM_MPU_SH_NON, 0, 0, 0),     /* Non-shareable, read/write, privileged, executable */
                      ARM_MPU_RLAR(TEST_REGION_1_END_ADDR, 1)                             /* Use Attr 1 */
                     );

    /*
     * Region 2 (SRAM Memory Space)
     * Start address = TEST_REGION_2_BASE_ADDR
     * Shareablity   = Non-shareable
     * Size          = TEST_REGION_2_SIZE
     * Permission    = Full access
     */
    ARM_MPU_SetRegion(2UL,
                      ARM_MPU_RBAR(TEST_REGION_2_BASE_ADDR, ARM_MPU_SH_NON, 0, 0, 0),     /* Non-shareable, read/write, privileged, executable */
                      ARM_MPU_RLAR(TEST_REGION_2_END_ADDR, 1)                             /* Use Attr 1 */
                     );

    /*
     * Region 3 (SRAM Memory Space)
     * Start address = TEST_REGION_3_BASE_ADDR
     * Shareablity   = Non-shareable
     * Size          = TEST_REGION_3_SIZE
     * Permission    = No write access
     */
    ARM_MPU_SetRegion(3UL,
                      ARM_MPU_RBAR(TEST_REGION_3_BASE_ADDR, ARM_MPU_SH_NON, 1, 0, 0),    /* Non-shareable, read-only, privileged, executable */
    ...
    printf("Press any key to test read access from memory region 1 (Flash Memory).\n");
    getchar();
    printf("\nRead value from 0x%08X is 0x%08X.\n", (uint32_t)TEST_REGION_1_BASE_ADDR, M32(TEST_REGION_1_BASE_ADDR));
    ...
    printf("Press any key to test read access from memory region 2 (SRAM Memory).\n");
    getchar();
    printf("\nRead value from 0x%08X is 0x%08X.\n", TEST_REGION_2_BASE_ADDR, M32(TEST_REGION_2_BASE_ADDR));
    ...
    printf("Press any key to test read/write access from memory region 3 (SRAM Memory).\n");
    printf("(It should trigger a memory fault exception.)\n");
    getchar();
    printf("\nRead value from 0x%08X is 0x%08X.\n", TEST_REGION_3_BASE_ADDR, M32(TEST_REGION_3_BASE_ADDR));
    /* Write data to address (SRAM_BASE + 0x4000) */
    printf("\nWrite 0 to 0x%08X.\n", TEST_REGION_3_BASE_ADDR);
    M32(TEST_REGION_3_BASE_ADDR) = 0;
}
