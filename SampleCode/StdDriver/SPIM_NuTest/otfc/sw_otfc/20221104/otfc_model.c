...
int check_data(uint32_t addr, uint32_t data, int err)
{
    uint32_t prdata;
...
void otfc_clock_power_setting()
{
    // unlock register protection
    outp32(PROT_BA, 0x59);
    outp32(PROT_BA, 0x16);
    outp32(PROT_BA, 0x88);
    // enable otfc0 and otfc1 clock enable
    outp32(CLK_BA + 0x288, (inp32(CLK_BA + 0x288) | 0x00000003));
}
...
void crypto_clock_power_setting()
{
    ...
    outp32(CLK_BA + 0x214, (inp32(CLK_BA + 0x214) | 0x00000001));
    // enable keystore clock enable
    outp32(CLK_BA + 0x264, (inp32(CLK_BA + 0x264) | 0x00000001));
    // enable SRAM0/1/2/3 clock enable
    outp32(CLK_BA + 0x2B8, (inp32(CLK_BA + 0x2B8) | 0x0000000F));
}
...
void otfc_reset()
{
    ...
    outp32(PROT_BA, 0x88);
    // enable otfc0 and otfc1 clock enable
    outp32(SYS_BA + 0x284, 0x00000003);
    printf("otfc reset !");
    outp32(SYS_BA + 0x284, 0x00000000);

}

//PR number, OTFC_PR1_KEY0, OTFC_PR1_KEY1, OTFC_PR1_KEY2, OTFC_PR1_KEY3, OTFC_PR1_NON0(scramble key), OTFC_PR1_NON1, OTFC_PR1_NON2, OTFC_PR1_NON3, ADDR
                ...
                uint32_t data0, uint32_t data1, uint32_t data2, uint32_t data3, uint32_t addr)
{
    ...
    if (pr_no == 0x0) taddr = ((addr & 0xFFFFFFF0) + 0x1) ^ data0;

    if (pr_no == 0x1) taddr = ((addr & 0xFFFFFFF0) + 0x2) ^ data0;

    if (pr_no == 0x2) taddr = ((addr & 0xFFFFFFF0) + 0x4) ^ data0;

    if (pr_no == 0x3) taddr = ((addr & 0xFFFFFFF0) + 0x8) ^ data0;

    //print("taddr before s-box= ");
    //printd(taddr);
...
int data_swap(uint32_t input)
{
    ...
    data_byte0 = (input & 0x000000FF) << 24;
    data_byte1 = (input & 0x0000FF00) << 8;
    data_byte2 = (input & 0x00FF0000) >> 8;
    data_byte3 = (input & 0xFF000000) >> 24;

    output = (data_byte0 | data_byte1 | data_byte2 | data_byte3);
