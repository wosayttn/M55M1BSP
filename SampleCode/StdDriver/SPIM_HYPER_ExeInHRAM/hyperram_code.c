...
void HyperRAM_TrainingDelayNumber(SPIM_T *spim)
{
    ...
    __attribute__((aligned(32))) uint8_t au8TrimPatten[32] =
    {
        ...
        0xFF, 0xDD, 0xFF, 0xFB, 0xFF, 0xFB, 0xBF, 0xFF,
        0x7F, 0xFF, 0x77, 0xF7, 0xBD, 0xEF,
    };
#ifdef DMM_MODE_TRIM
    ...
    for (u8RdDelay = 0; u8RdDelay <= SPIM_HYPER_MAX_LATENCY; u8RdDelay++)
    {
        ...
        if (memcmp(au8TrimPatten, g_au8DestArray, u32TestSize))
        {
    ...
    printf("final Set Delay Step Num : %d\r\n", u8RdDelayRes[u8RdDelayIdx]);
