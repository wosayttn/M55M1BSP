...
for %%i in (%key_file%) do set key_basename=%%~ni
...
if not exist %call_path%\key_basename.pub (
    %script_path%\imgtool getpub -k %call_path%\%key_file% > %call_path%\%key_basename%.pub
)
...
-S 0x10000 -s 0 %bin_file% %script_path%\%bin_basename%_signed.bin
